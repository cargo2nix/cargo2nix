...
fn version_req(path: &Path) -> VersionReq {
...
fn write_to_file(file: impl AsRef<Path>) {
    ...
    if file.exists() {
        let vers_req = version_req(file);
        if !vers_req.matches(&version()) {
            println!(colorify!(red_bold: "\nVersion requirement {}"), vers_req);
            println!(
...
fn generate_cargo_nix(mut out: impl io::Write) {
    ...
    let display = DisplayFn(|f: &mut fmt::Formatter| {
        writeln!(f, "# This file was @generated by cargo2nix {}.", version())?;
        writeln!(f, "# It is not intended to be manually edited.")?;
        ...
        {
            let mut f = f.indent(2);
            writeln!(f, "{} = \"{}\";", VERSION_ATTRIBUTE_NAME, version())?;
            writeln!(f, "workspace = {{")?;
            for pkg in root_pkgs.iter() {
                writeln!(
                    f.indent(2),
