...
impl Predicate for Any {
    ...
    fn test_open(&self, platform: &CratePlatform) -> MaybeBool {
        use MaybeBool::*;
        self.0.iter().fold(MaybeBool::False, |maybe, p| {
            match (maybe, p.test_open(platform)) {
                (
                    Maybe {
                        mut positive,
                        mut negative,
...
impl Predicate for All {
    ...
    fn test_open(&self, platform: &CratePlatform) -> MaybeBool {
        use MaybeBool::*;
        self.0.iter().fold(MaybeBool::True, |maybe, p| {
            match (maybe, p.test_open(platform)) {
                (
                    Maybe {
                        mut positive,
...
impl Predicate for Not {
    ...
    fn test_open(&self, platform: &CratePlatform) -> MaybeBool {
        use MaybeBool::*;
        match self.0.test_open(platform) {
...
impl Predicate for Key {
    fn test(&self, platform: &CratePlatform) -> bool {
        ...
        match self {
            ...
            Feature(feature) => platform.crate_features.iter().any(|f| f == feature),
            _ => false,
        }
    }
    fn test_open(&self, platform: &CratePlatform) -> MaybeBool {
